...
> 文档状态：V4 实施与发布验收版，已合并 2026-08-27 安吉星生产实跑、现场反馈、`0.3.93` 代码审计、`0.3.94` Hotfix 事实与 `0.3.95` 本地 RC 配额心跳补丁
> 首次编制：2026-08-24；本次修订：2026-08-27
...
`codex/hotfix-extension-storage-heartbeat-20260827` 当前本地 RC 已完成以下闭环：
...
当前证据是 Extension/Server 合同与极限测试、Node 24 和生产兼容 Node 18 各 `1560/1560` 全仓回归、两套运行时各 `23/23` 隔离 PostgreSQL 集成。它仍是本地 release candidate：尚未提交、推送、部署，也尚未在雪人真实 Edge Profile reload 后验证配额恢复、`0.3.95` 完整心跳和控制面双时间戳；因此不能写成生产已解决。
