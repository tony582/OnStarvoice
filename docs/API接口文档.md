...
> P2-A1 合并与发布来源锚点：`0ec5508`；2026-08-14 生产核对值为 Extension `0.3.83`、数据库迁移 `066_tenant_comment_risk_attention.sql`。生产中纳入 Git 的 Server 文件，仍是 `236aa4d` 完整基线叠加 4 个来自 `0ec5508` 的白名单文件，不是完整 Git 树；精确发布清单见 [开发运行与生产发布手册](./开发运行与生产发布手册.md#14-2026-08-14-p2-a1-限定生产快照)。
