...
相关性前置筛选用于在打开详情页之前，同时判断列表中的作品是否匹配当前搜索关键词、是否属于租户整体监测范围。它的目标是减少无效详情页访问和风控暴露，而不是替代最终舆情判断。
...
- 当前提示词版本为 `prefilter-list-v3`；
- `modelDecision/queryMatch` 只描述当前关键词匹配，`tenantRelevance/brandMatch` 描述租户整体监测相关性；
- 只有当前关键词高置信度不匹配、租户整体明确无关、整体匹配分足够低且没有投诉、故障、救援等保护信号时，才允许跳过增强；
- 采集后的最终相关性和情感判断使用租户品牌背景与全部监测主题；整体无关不再等同于“中性”，而是情感不适用。
