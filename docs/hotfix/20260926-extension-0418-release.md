...
### 服务端（47.103.125.200，22:29:48 已上线）

部署材料在 `/opt/onstarvoice-private/releases/extension-0418-fe74ec5-20260926/`：
...
CI 通过后，本会话一度连不上生产 SSH：权限策略要求用户在对话里点名生产主机。用户点名后执行：
...
`deploy.sh` 的步骤：重新预检并加锁；解包并逐文件核对哈希；备份三份文件；先放入 0.4.18 包，再原子替换三份文件，然后 `pm2 restart onstarvoice`；最后逐项核对，任何一步失败都会自动回滚。核对项：

- `health/ready`、`health/live` 通过，进程确实已重启；
- 磁盘上的文件就是本次发布的版本；
- `/api/update-manifest` 宣告 0.4.18；
- 新包下载后与上面的 SHA-256 逐字节一致，0.4.17 包仍可下载，`public-downloads/` 恰好多出新包一个文件；
- `/changelog` 显示 0.4.18；
- 经公网再下载一次新包比对（只提示，不回滚）。

执行结果：

- 22:29:46 记下部署前基线：PM2 `onstarvoice` 为 PID 469496；错误日志 316729 行，其中 `DB_CAPACITY_UNAVAILABLE` 5992 次。
- 预检通过。22:29:48 完成切换并重启，新 PID 为 473727。脚本内各项核对全部通过，经公网下载的包与 SHA-256 一致，22:29:54 退出码为 0。
- 生产现在的服务端代码为 `fe74ec5`。

部署后核对（22:31–22:35）：

- **公网**：`/api/update-manifest` 宣告 0.4.18，`minSupportedVersion` 仍为 0.3.51，不强制升级；`/changelog` 中 0.4.18 标为最新。
- **错误日志**：
  - 新增 32 行，是同一条报错：`/agent/liveness` 的事务遇到 `canceling statement due to lock timeout`（`55P03`）。
  - 这是长期存在的现象：部署前日志里已有 952 次，这条路由占 24 次。改动的三份文件也不访问数据库。
  - `DB_CAPACITY_UNAVAILABLE` 仍为 5992 次。
  - 此后到 22:34:52 没有再新增，PM2 没有再重启。
- **节点心跳**：
  - 部署前 30 分钟内出现过的 18 个在用节点中，17 个在重启后约 1 分 45 秒内恢复心跳，手机 Runner 也在其中。
  - 到 22:34:52，已有 13 个节点上报 0.4.18。
  - 唯一没回来的是 `Chrome · macOS`（主机 `76fb804d`，0.4.17，07-28 注册）。它最后一次心跳在 22:28:45，比重启早约 1 分钟。同一主机上另有一行 `Chrome · macOS-Tony`（0.3.62）旧注册。它不是本机的成都、北京、上海、重庆配置，需要看那台电脑的 Chrome 是否还开着。
- **西瓜那条卡住的补详情**（`12013e31`，`discovered_post_capture`）：服务器上已是 `canceled`，时间 22:29:40，早于服务端切换。
  - 西瓜重载到 0.4.18 后，由后台按修复逻辑结束并上报。
  - 服务端结果为 `detail_canceled`、`reportedStatus: canceled`。这个错误码是服务端按「已停止」换算的，见 `server/services/capture-discovery/detail-projection.js`。

发布材料与前后对比都在上面的发布目录里：`backup/server/…`、`before-*`/`after-*`、`runtime.json`、`pre-deploy-errlog.txt`。

## 节点重载

- 重载会中断该浏览器配置里正在跑的采集，请在节点空闲或停稳后进行。
- 西瓜已重载。那条卡住的「手机发现作品补详情」（作品 7689644539300486906）已于 22:29:40 自动结束为「已停止」并上报，见上文。
- 其他节点：本机的 Chrome/Edge 配置逐个重载；其他电脑上的节点用侧栏的更新提示或下载地址升级。22:34:52 时 18 个节点中已有 13 个上报 0.4.18。
