...
test('admin waits visibly, retries one safety challenge across Agents, and escalates repeated safety blocks', async () => {
  ...
  assert.match(orchestration, /尚未满足服务端接力条件/u)
  assert.match(orchestration, /工作项已释放 · 恢复/u)
  assert.match(orchestration, /其他空闲 Agent 可立即领取/u)
  assert.match(orchestration, /仅隔离/u)
  assert.match(orchestration, /原账号可继续领取其他关键词/u)
  assert.match(orchestration, /真正下发后会显示目标 Agent 和命令状态/u)
  assert.match(orchestration, /等待空闲 Agent 心跳领取/u)
  assert.match(orchestration, /最终分配以服务端提交时的实时状态为准/u)
  assert.doesNotMatch(orchestration, /自动尝试已耗尽/u)
  assert.doesNotMatch(orchestration, /恢复阻塞实时状态/u)
  assert.doesNotMatch(orchestration, /原 Agent 冷却/u)
  assert.doesNotMatch(orchestration, /等待原 Agent 关闭确认/u)
  assert.doesNotMatch(orchestration, /本地关闭证明/u)
  assert.doesNotMatch(orchestration, /recovery\.sourceAgentHoldUntil/u)
  assert.doesNotMatch(orchestration, /recovery\.nextEvaluationAt/u)
