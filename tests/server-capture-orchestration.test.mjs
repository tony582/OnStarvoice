...
test('sequential patrol stays one keyword item and is executed inside one Agent task', async () => {
  const [scheduler, route, agent, controlOutcomeProjection] = await Promise.all([
  ...
  const refreshStart = controlOutcomeProjection.indexOf(
    'export async function refreshOrchestrationParentTask',
  );
  const refreshEnd = controlOutcomeProjection.indexOf(
    'export async function projectOrchestrationChildControlOutcome',
    refreshStart,
  );
  const refresh = controlOutcomeProjection.slice(refreshStart, refreshEnd);
