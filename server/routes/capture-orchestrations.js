...
import {
  ...
  elasticQueueOwnsRetry,
  itemRequiresManualSafetyAction,
  publicPendingRetryItem,
} from '../modules/capture/application/pending-orchestration-retry.js';
import {
  loadRetryAgentCandidates,
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await withTransaction(async tx => {
        ...
        // this parent before taking any Agent, source, parent or item locks.
        await tx.execute(
          'SELECT pg_advisory_xact_lock(hashtext($1), hashtext($2))',
