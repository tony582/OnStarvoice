...
import {customerDailySummaryRows} from './customer-daily-report-presentation.js';

const FONT = fileURLToPath(new URL('../assets/daily-report/StarVoiceDailyTable.ttf', import.meta.url));
const WIDTHS = [160, 180, 160, 130, 130, 170, 175, 175];
const WIDTH = WIDTHS.reduce((sum, value) => sum + value, 0);
const HEADER = 52;
const ROW = 62;
const HEIGHT = HEADER * 2 + ROW * 2;
...
export function renderCustomerDailySummarySvg(snapshot) {
  ...
  rows.forEach((row,i)=>row.forEach((value,j)=>cell(xs[j],HEADER*2+i*ROW,WIDTHS[j],ROW,value)));
  parts.push('</g></svg>');
  return parts.join('');
}
