...
function renderSummaryHtml({snapshot, title}, handling, {email = false} = {}) {
  ...
  // Word-based Outlook needs a constrained table cell rather than a div max-width.
  const outlookOpen = email ? '<!--[if mso]><table role="presentation" width="600" border="0" cellpadding="0" cellspacing="0" style="width:450pt;border-collapse:collapse;table-layout:fixed;mso-table-lspace:0pt;mso-table-rspace:0pt"><tr><td width="600" style="width:450pt;padding:0;border:0"><![endif]-->' : '';
  const outlookClose = email ? '<!--[if mso]></td></tr></table><![endif]-->' : '';
  return `<h2>${esc(title)}</h2>${customerDailySummaryBasis(snapshot) ? `<p class="notes">${customerDailySummaryBasis(snapshot)}</p>` : ''}<div class="table-wrap">${outlookOpen}<table${tableStyle} class="${email ? 'email-summary ' : ''}${handling ? 'monthly' : ''}" aria-label="${esc(title)}"><thead>${email ? emailCells(header) : header}</thead><tbody>${email ? emailCells(body) : body}</tbody></table>${outlookClose}</div>`;
}
