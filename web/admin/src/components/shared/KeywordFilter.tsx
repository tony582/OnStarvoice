        ...
        <div className="responsive-filter-popover absolute left-0 top-full z-50 mt-1 w-60 rounded-xl border border-border bg-card p-2 shadow-lg lg:z-30 lg:rounded-lg">
          <div className="relative mb-1.5">
            ...
            <input value={q} onChange={e => setQ(e.target.value)} placeholder="筛选关键词…"
              className="h-10 w-full rounded-md border border-border bg-background pl-8 pr-2 text-[12px] outline-none focus:border-primary lg:h-7 lg:pl-7" />
          </div>
          {value.length > 0 && (
          ...
          <div className="max-h-60 overflow-y-auto">
            {filtered.length === 0 ? (
              ...
              <label key={it.keyword} className="flex min-h-10 cursor-pointer items-center gap-2 rounded-md px-2 py-2 text-[12px] hover:bg-accent lg:min-h-0 lg:px-1.5 lg:py-1.5">
