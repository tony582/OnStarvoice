...
function CustomerDailyReportWorkspace() {
  ...
  async function saveSummary() {
    ...
    if (snapshot && isMonthlySummary(snapshot)) {
      try { summary = parseMonthlyDraft(snapshot.summary.rows!, summaryDraft, isHandlingSummary(snapshot) || isCollectionHandlingSummary(snapshot)) }
    ...
    {loading ? <div role="status" className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white py-20 text-sm text-slate-500"><Loader2 className="h-5 w-5 animate-spin" />正在读取日报…</div> : !snapshot ? <div className="rounded-xl border border-slate-200 bg-white px-6 py-16 text-center">
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        ...
        <article className="space-y-8 p-5 sm:p-7" aria-label="客户日报正文">
          ...
          <SummaryTable snapshot={snapshot} draft={summaryDraft} canEdit={canWrite()} busy={!!busy || anyProcessing} onEdit={() => { setSummaryDraft(isMonthlySummary(snapshot) ? monthlyDraftFromRows(snapshot.summary.rows!, isHandlingSummary(snapshot) || isCollectionHandlingSummary(snapshot)) : summaryToDraft(snapshot.summary)); setNotice(null); summaryRequest.current = null }} onChange={(period, field, value) => { summaryRequest.current = null; setSummaryDraft(draft => draft ? { ...draft, [period]: { ...draft[period], [field]: value } } : null) }} onCancel={() => { setSummaryDraft(null); summaryRequest.current = null; setNotice(null) }} onSave={() => void saveSummary()} />
          {isHandlingSummary(snapshot) && snapshot.collectionSummary && <CollectionSummaryTable snapshot={snapshot} />}
          <ReportSection title={`${isHandlingSummary(snapshot) && snapshot.collectionSummary ? '三' : '二'}、7天内热度值≥200的负面帖子`}>
...
function CollectionSummaryTable({ snapshot }: { snapshot: DailySnapshot }) {
  const summary = snapshot.collectionSummary!
  const fields = ['monitor', 'sdb', 'positive', 'neutral', 'negative'] as const
  const labels = { monitor: '采集量', sdb: 'SDB范畴', positive: '正面', neutral: '中性', negative: '负面' }
  const rows = summary.rows ? visibleMonthlyRows(summary.rows) : [{ date: summary.dayDate || snapshot.reportDate, isWorkingDay: true, counts: summary.day }]
  return <section>
    <h4 className="mb-4 text-base font-semibold text-slate-900">二、实际采集量</h4>
    <div className="overflow-x-auto border border-slate-300">
      <table className="w-full min-w-[560px] border-collapse text-center text-xs tabular-nums" aria-label="逐日实际采集量与月累计">
        <thead className="bg-[#101416] text-white"><tr className="[&>th]:border-b [&>th]:border-r [&>th]:border-slate-300 [&>th]:px-3 [&>th]:py-3 [&>th]:font-semibold [&>th:last-child]:border-r-0"><th scope="col">日报日期</th>{fields.map(field => <th key={field} scope="col">{labels[field]}</th>)}</tr></thead>
        <tbody>{rows.map(row => <tr key={row.date} className="border-b border-slate-300 [&>td]:border-r [&>td]:border-slate-300 [&>td]:px-3 [&>td]:py-2.5 [&>td:last-child]:border-r-0"><th scope="row" className="border-r border-slate-300 px-3 py-2.5 font-normal">{row.date.split('-').map(Number).join('/')}</th>{fields.map(field => <td key={field}>{Number(row.counts[field] ?? 0).toLocaleString()}</td>)}</tr>)}</tbody>
        <tfoot><tr><td colSpan={6} className="border-b border-slate-300 bg-[#f2f2f2] px-3 py-3 text-left sm:text-center">本月采集去重累计</td></tr><tr className="[&>td]:border-r [&>td]:border-slate-300 [&>td]:px-3 [&>td]:py-3 [&>td:last-child]:border-r-0"><th scope="row" className="border-r border-slate-300 px-3 py-3 font-normal">MTD</th>{fields.map(field => <td key={field} aria-label={`采集 MTD ${labels[field]}`}>{Number(summary.mtd[field] ?? 0).toLocaleString()}</td>)}</tr></tfoot>
      </table>
    </div>
    <p className="mt-3 text-xs leading-6 text-slate-500">首次成功入库的新帖去重统计，复采不重复计数；休息日采集内容合并至下一工作日。{snapshot.collectionStartAt && snapshot.collectionCutoffAt ? `本期采集范围：${dailyTime(snapshot.collectionStartAt)} 至 ${dailyTime(snapshot.collectionCutoffAt)}。` : ''}</p>
  </section>
}
...
function coldSectionTitle(posts: DailyPost[], section = '三') {
  ...
  return <ol className="divide-y divide-slate-200 border-y border-slate-200">{posts.map((post, index) => {
    ...
    return <li key={`${post.recordId}-${index}`} className="flex gap-3 py-4 sm:gap-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1.5 text-xs leading-5 text-slate-500">{platforms[post.platform] || post.platform || '未知平台'}{kind === 'cold' && post.isHistorical === true && <span className="ml-2 inline-flex rounded bg-slate-100 px-1.5 py-0.5 text-[11px] text-slate-600">历史帖</span>}{kind === 'heat' && <>｜热度 {post.heatText || post.heat?.toLocaleString() || '待核对'}｜较昨日 {(post.comparisonText || '暂无对比').replace(/^较昨日\s*[:：]?\s*/, '').trim() || '暂无对比'}</>}</p>
