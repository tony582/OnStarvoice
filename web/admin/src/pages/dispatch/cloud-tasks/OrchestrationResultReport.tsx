...
import { orchestrationCurrentExecution, orchestrationItemTiming, orchestrationResultEvidence, orchestrationResultRecovery, orchestrationSearchStepLabel, resultCount, resultMessage, resultObject } from './task-result-presentation.mjs'
...
function unfinishedEvidence(item: OrchestrationItemRecord, execution: OrchestrationExecutionRecord | undefined, expected: number, searchPasses: string[]) {
  const reasons = []
  const evidence = orchestrationResultEvidence(item, execution, expected, searchPasses)
  if (evidence.childIncomplete) reasons.push(`本次执行${STATUS_LABELS[String(execution?.status)] || execution?.status}`)
  if (evidence.unfinishedStepLabels.length > 0) reasons.push(`未完成：${evidence.unfinishedStepLabels.join('、')}`)
...
export function OrchestrationResultReport({ items, executions, agents, attempts, expectedSearchPasses, searchPasses = [], parentStatus = '', automaticRecovery = false, now = Date.now(), recoveryActions = {} }: Props) {
  ...
  const findings = items.flatMap(item => {
    ...
    const reasons = unfinishedEvidence(item, execution, expectedSearchPasses, searchPasses)
    const evidence = orchestrationResultEvidence(item, execution, expectedSearchPasses, searchPasses)
    const recovery = orchestrationResultRecovery(item, execution, {parentStatus, automaticRecovery, now, evidence})
    return reasons.length ? [{item, reasons, recovery}] : []
  })
  return <div className="mt-4 space-y-4">
    {findings.length > 0 && <section className="rounded-xl border border-status-orange/30 bg-status-orange/5 p-4" aria-label="结果核对提醒">
      ...
      <p className="mt-2 text-xs leading-5 text-muted-foreground">以下列出尚未完成的步骤和当前恢复状态；已恢复的问题保留在历次尝试中。</p>
      <ul className="mt-2 space-y-1 text-xs leading-5">{findings.map(({item, reasons, recovery}) => <li key={item.id}><strong>{itemLabel(item)}</strong>：{reasons.join('；')}。<strong>{recovery.label}</strong>{recovery.message ? `：${recovery.message}` : ''}</li>)}</ul>
    </section>}
    <section className="overflow-hidden rounded-xl border border-border/70">
      ...
      {items.length === 0 ? <p className="p-4 text-xs text-muted-foreground">此任务没有保留工作项明细。</p> : <div className="divide-y divide-border/60">{items.map((item, index) => {
        ...
        const issues = unfinishedEvidence(item, execution, expectedSearchPasses, searchPasses)
        const error = recovery.currentError
        const recovering = ['queued', 'running', 'assigned', 'blocked'].includes(recovery.state)
        const recoveryAction = recovery.state !== 'completed' && recovery.state !== 'exhausted' ? recoveryActions[item.id] : undefined
        return <article key={item.id} className="p-4">
          <div className="flex flex-wrap items-start justify-between gap-2"><div className="min-w-0 flex-1"><span className="text-[10px] text-muted-foreground">{index + 1} · {itemType(item)} · {PLATFORM_LABELS[item.platform] || item.platform}</span><h4 className="mt-1 break-words text-sm font-semibold">{itemLabel(item)}</h4></div><span className="rounded-md bg-muted px-2 py-1 text-[11px]">{recovery.state === 'completed' ? STATUS_LABELS[item.status] || item.status : recovery.label}</span></div>
          ...
          {error && <p className={`mt-2 text-xs leading-5 ${recovering ? 'text-muted-foreground' : 'text-status-red'}`}>{recovering ? '上次未完成原因：' : ''}{error}</p>}
          {issues.length > 0 && <p className="mt-2 text-xs leading-5 text-amber-800">结果核对：{issues.join('；')}</p>}
          {recovery.message && <p className="mt-2 text-xs leading-5 text-muted-foreground">当前状态：{recovery.message}</p>}
          {recoveryAction && <a className="mt-2 inline-block text-xs text-primary underline underline-offset-2" href={recoveryAction.href}>{recoveryAction.label}</a>}
          ...
          {(steps.length > 0 || history.length > 0) && <details className="mt-2 rounded-lg bg-muted/25 p-3" open={issues.length > 0}><summary className="cursor-pointer text-xs text-primary">步骤与历次尝试</summary>
            {steps.length > 0 && <div className="mt-2 space-y-2">{steps.map((step, stepIndex) => <p key={stepIndex} className="text-[11px] leading-5">{orchestrationSearchStepLabel(Number(step.round || stepIndex + 1), searchPasses)} · {STATUS_LABELS[String(step.status)] || String(step.status || '未记录')} · 设备报告保存 {resultCount(step.savedCount) ?? '未记录'} · {formatTime(String(step.finishedAt || ''))}{!['completed', 'completed_with_warnings'].includes(String(step.status)) && resultMessage(step.error) ? ` · ${resultMessage(step.error)}` : ''}</p>)}</div>}
    ...
    <section className="overflow-hidden rounded-xl border border-border/70">
      ...
      {executions.length === 0 ? <p className="p-4 text-xs text-muted-foreground">未保留节点执行记录。</p> : <div className="divide-y divide-border/60">{executions.map((execution, index) => {
        ...
        const error = ['completed', 'completed_with_warnings'].includes(String(execution.status)) ? '' : resultMessage(execution.error)
