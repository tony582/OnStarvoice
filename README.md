...
>
> 2026-08-15，P2-B 已通过 PR #21 合并到 `main`，merge commit 为 `7c3e47ef1717a3fc498d63a1e3b7952e9100ecee`，但**尚未部署生产**；线上不能被描述为已经配置 `PROCESS_ROLE` 或已受角色锁保护。P2-C 实现提交 `bc4d665` 已推送至分支 `codex/architecture-modernization-p2c-process-runtime`，并创建 [Draft PR #22](https://github.com/tony582/OnStarvoice/pull/22)；尚未合并，也未部署。
>
...
该候选已通过 Node 24 与生产 Node 18 的 1250/1250 回归、在两版 Node 下各 13/13 的隔离 PostgreSQL 集成，以及语法、拓扑、前端、仓库卫生和 92 文件 Extension 生产快照门禁；门禁前后工作区状态一致。实现已进入 Draft PR #22，但尚未合并、未部署，不是生产操作指令；拆分上线继续受 P2-D、P2-E 和另行发布授权阻断。
