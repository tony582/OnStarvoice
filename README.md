...
StarVoice 是一套面向企业客户的多平台社交舆情采集、任务调度、智能研判与业务处置系统。系统由浏览器 Extension、云端服务、PostgreSQL、管理后台和独立数据看板组成，支持小红书、抖音等平台的内容采集、评论巡查、多 Agent 编排、负面帖子跟踪、官方账号评论巡查、社交账号负载管理以及工单处置。
...
>
> 2026-08-15，[PR #23](https://github.com/tony582/OnStarvoice/pull/23) 已合并，P2-D 进入 `main@7bf7ab1f91a41fc6327c1070e345f1ce8055974b`。P2-B/P2-C/P2-D **当前均未部署生产**；生产仍为迁移 066、单一 `all`，未启用 split。P2-E-L 本机隔离轮次已完成：最新 diff 在本机 PostgreSQL 17 专用空业务库、随机 schema、真实 Node OS 入口进程与非本机 outbound guard 基础上，增加 topology `1/1` → 其余 integration `25/25` 的两阶段 runner，以及容忍瞬时 CI 探测但拒绝持久会话的连续稳定空闲快照门禁；Node 24/18.20.8 的定向 topology 均为 `1/1`，完整 PostgreSQL runner 均以原始 TAP `1/1 + 25/25` 通过并可汇总为 `26/26`，非 PostgreSQL 回归均为 `1289/1289`，工程门禁通过。每轮清理前连接、P2-E/P2-D schema 与 advisory lock 均为 0，精确专用库均已删除。P2-E-H 尚未执行，因此 P2-E 整体仅部分完成，生产 split 阻断保留。
>
...
当前标准本地启动仍使用兼容 `all` 入口。`main@7bf7ab1` 已包含 `api`、`scheduler`、`ai-media` 三个独立入口，以及兼容 `/api/health` 的 `/api/health/live`、`/api/health/ready` 语义；响应后继续执行的进程内后台工作也已纳入统一的有界 drain。这些能力尚未部署生产。
