...
export async function captureSingleNote() {
  ...
  try {
    // Only the operator-confirmed REDNote/Xiaohongshu protection pages stop
    // the batch. Ordinary post copy mentioning security must not count.
    const xhsSecurityEvidence = detectXhsSecurityPage({
      title: document.title,
      text: document.body?.innerText,
      url: location.href,
    });
    if (xhsSecurityEvidence) {
      return {
        ok: false,
        ...
        error: xhsSecurityFailure(xhsSecurityEvidence),
      };
    }
